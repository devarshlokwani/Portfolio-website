...
/** Outer badge and inner nut, per size. */
const SIZES = {
  md: 'h-11 w-11',
  lg: 'h-16 w-16',
  xl: 'h-20 w-20',
...
interface GearBadgeProps {
  /**
   * Whether whatever owns this badge is hovered. The trigger is the owner,
   * not the badge: at forty-four pixels the badge is a small target for
   * something meant to respond to the thing around it being read. Left
   * false where there is nothing to hover, and the nut simply idles.
   */
  active?: boolean
  size?: keyof typeof SIZES
  /**
   * Drops the ringed disc and draws the nut on its own. On a card the ring
   * matches the badges beside it; standing alone in open space there is
   * nothing for it to match, and the ring just reads as a button that does
   * not do anything.
   */
  bare?: boolean
}
 ...
 */
export function GearBadge({ active = false, size = 'md', bare = false }: GearBadgeProps) {
  ...
  const nut = (
    <svg
      ...
      className={`transition-opacity duration-300 ease-out ${NUT[size]}`}
    >
      <g ref={nutRef}>
  ...
  if (bare) return <span aria-hidden="true">{nut}</span>

  return (
    <span
      aria-hidden="true"
      className={`relative flex shrink-0 items-center justify-center rounded-full border border-border bg-bg ${SIZES[size]}`}
    >
